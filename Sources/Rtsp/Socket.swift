...
//    private let lock = DispatchQueue(label: "socket.sync")
//    private let readLock = DispatchQueue(label: "socket.sync.read")
    ...
    public init(tlsEnabled: Bool, host: String, port: Int) {
        ...
        let op1 = tlsEnabled ? NWProtocolTLS.Options() : nil
        let parameters = NWParameters(tls: op1, tcp: options)
